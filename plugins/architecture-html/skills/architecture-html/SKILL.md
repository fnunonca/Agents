...
- `<pre class="mermaid">` envuelto en `<div class="seq-viewport" id="seq-viewport">` dentro de `.seq-canvas`, y todo eso dentro de un wrapper externo `.rf-wrapper` con `id="seq-wrapper"` — el wrapper provee la altura (~900px), el borde y el fondo, y es el target de `requestFullscreen()`. El `.seq-canvas` lleva también la clase `.rf-canvas` (para que el toolbar genérico lo reciba como `canvas`) y es `position: absolute; inset: 0; overflow: hidden; background: transparent; border: none; margin: 0; cursor: grab` (y `.is-panning { cursor: grabbing }`) — llena el wrapper sin marco propio. **Importante**: NO le pongas `height` fijo (antes era `720px` y dejaba franja muerta abajo + doble marco en fullscreen). El `.seq-viewport` es `position: absolute; top: 0; left: 0; width: max-content; transform-origin: 0 0`. La regla `.seq-viewport svg { display: block; max-width: none !important }` neutraliza cualquier `max-width` heredado. Como cinturón, agrega `#seq-wrapper:fullscreen, #seq-wrapper:-webkit-full-screen { height: 100vh }` para que el wrapper crezca al viewport completo en fullscreen (el `.seq-canvas` lo sigue automáticamente vía `inset: 0`).
